...
public class TriangularMatrix {
    public static void main (String[] args) {
        ...
        int sumOfUpperDigonal=0;
        int sumOfLowerDigonal=0;
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if(i<=j){
                    sumOfUpperDigonal+=arr[i][j];
                }
                if(i>=j){
                    sumOfLowerDigonal+=arr[i][j];
                }
            }
        }
        System.out.println(sumOfUpperDigonal+" "+sumOfLowerDigonal);
    }
}
